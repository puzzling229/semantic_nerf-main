...
class ReplicaDatasetCache(Dataset):
    def __init__(self, data_dir, train_ids, test_ids, train_sem_ids , img_h=None, img_w=None):
        ...
        self.train_sem_ids = train_sem_ids
        self.train_sem_num = len(train_sem_ids)
        self.img_h = img_h
        self.img_w = img_w
       ...
       # training samples
        ...
        for idx in train_ids:
            ...
            mask_sem = cv2.imread(self.semantic_list[idx], cv2.IMREAD_UNCHANGED)
            # semantic = cv2.imread(self.semantic_list[idx*5], cv2.IMREAD_UNCHANGED)
            if self.semantic_instance_dir is not None:
            ...
            if (self.img_h is not None and self.img_h != image.shape[0]) or \
                ...
                mask_sem = cv2.resize(mask_sem, (self.img_w, self.img_h), interpolation=cv2.INTER_NEAREST)
            ...
            self.train_samples["mask_sem"].append(mask_sem)
        ...
        #semantic samples
        for idx in train_sem_ids:
            semantic = cv2.imread(self.semantic_list[idx], cv2.IMREAD_UNCHANGED)
            if (self.img_h is not None and self.img_h != semantic.shape[0]) or \
                    (self.img_w is not None and self.img_w != semantic.shape[1]):
                semantic = cv2.resize(semantic, (self.img_w, self.img_h), interpolation=cv2.INTER_NEAREST)
            self.train_samples["semantic"].append(semantic)
