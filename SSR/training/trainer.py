...
class SSRTrainer(object):
    ...
    def prepare_data_replica(self, data, gpu=True):
        ...
        self.train_sem_ids = data.train_sem_ids
        self.test_ids = data.test_ids
        self.mask_ids = data.mask_ids

        self.num_sem_train = data.train_sem_num
        ...
        self.mask_sem = torch.from_numpy(train_samples["mask_sem"])
        ...
        self.train_semantic_clean_scaled = F.interpolate(torch.unsqueeze(self.mask_sem, dim=1).float(),
                                                            scale_factor=1/self.config["render"]["test_viz_factor"], 
    ...
    def sample_data(self, step, rays, h, w, no_batching=True, mode="train"):
        ...
        if mode == "train":
            ...
            sample_sem_num=self.num_sem_train
        elif mode == "test":
        ...
        if no_batching:  # sample random pixels from one random images
            _, index_hw = sampling_index(self.n_rays, num_img, h, w)
            index_batch = step % 180
            sem_index_batch = index_batch * 5
            ...
            if self.enable_semantic and (sem_index_batch in self.train_sem_ids):
                set_index= sem_index_batch / 25
                ...
                gt_semantic = semantic.reshape(sample_sem_num, -1)[set_index, index_hw].reshape(-1)
                gt_semantic = gt_semantic.cuda()
            else:
                sematic_available_flag = 0
                gt_depth = None
                gt_semantic = None
        ...
        if self.enable_semantic and (sem_index_batch in self.train_sem_ids):
            ...
            return sampled_rays, sampled_gt_rgb, gt_depth, gt_semantic,sematic_available_flag
    ...
    def render_path(self, rays, save_dir=None):
        ...
        for i, c2w in enumerate(tqdm(rays)):
            ...
            if save_dir is not None:
                ...
                imageio.imwrite(disp_filename, disp, format="png")

                imageio.imwrite(depth_filename, dep_mm, format="png")
